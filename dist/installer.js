 ...
 */
export function getOMVersion(versionInput) {
 ...
 */
async function aptInstallOM(packages, version, bit, useSudo) {
    let sudo;
    if (useSudo) {
        sudo = 'sudo';
    }
    ...
    await exec.exec(`/bin/bash -c "${sudo} rm -f /etc/apt/sources.list.d/openmodelica.list /usr/share/keyrings/openmodelica-keyring.gpg"`);
    // Add OpenModelica PGP public key
    await exec.exec(`/bin/bash -c "curl -fsSL http://build.openmodelica.org/apt/openmodelica.asc ${'|'} ${sudo} gpg --dearmor -o /usr/share/keyrings/openmodelica-keyring.gpg"`);
    await exec.exec(`/bin/bash -c "echo deb [arch=${arch} signed-by=/usr/share/keyrings/openmodelica-keyring.gpg] \
    ...
    for (const pkg of packages) {
        if (version.type === 'nightly' || !version.aptname) {
            ...
            await exec.exec(`/bin/bash -c "${sudo} apt-get install ${pkg}=${version.aptname} -V -qy"`);
        }
    }
}
/**
...
async function winInstallOM(version, bit) {
    ...
    const installer = await util.downloadCachedSync(version.address, 'tmp', version.version === 'nightly');
    if (bit !== version.arch) {
 ...
 */
export async function installOM(packages, version, architectureInput) {
    switch (osPlat) {
        case 'linux':
 ...
 */
export async function showVersion(program) {
    const out = await exec.getExecOutput(program, ['--version']);
    if (out.exitCode !== 0) {
        core.debug(`Error message: ${out.stderr}`);
        core.setFailed(Error(`${program} could not be installed properly. Exit code: ${out.exitCode}`));
    }
 ...
 */
export async function installLibs(librariesInput) {
 ...
 * @param useSudo       true if root rights are required.
 */
export async function installOmcDiff(useSudo) {
    switch (osPlat) {
        case 'linux':
            break;
        case 'win32':
            ...
            return;
        default:
            throw new Error(`omc-diff not available for platform ${osPlat}. Open a feature request on https://github.com/OpenModelica/omc-diff.`);
    }
